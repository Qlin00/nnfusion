...
def gen_key(data, dtype="float"):
    ...
    TESAID = data["miscs"]["TESAID"] if "miscs" in data and "TESAID" in data["miscs"] else -1

    if TESAID != -1:
        return op_type + f"_TESAID_{TESAID}"
...
def gen_config(op_type, kernel):
    ...
    elif (op_type == "Fused_Dot_Add"):
        config["in_shape"] = [kernel["parameters"]
                              ["arg0_shape"], kernel["parameters"]["arg1_shape"]]
        config["out_shape"] = [kernel["parameters"]["out_shape"]]
        config[
            "function_signature"] = "extern \"C\" __global__  void (float* __restrict__ input0,  float* __restrict__ input1,  float* __restrict__ input2,  float* __restrict__ output0)"
    elif (op_type == "Relu"):
