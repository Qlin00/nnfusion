    ...
    : CudaLibEmitter(ctx)
{
    ...
    tag << "CusparseLT emitter initilization";
    custom_tag = tag.str();
}
...
LanguageUnit_p cuda::CusparseLT::emit_function_body()
{
    ...
    if(dtype==element::f32){
        ...
        lu << "auto type  = CUDA_R_32F;\n";
        lu << "auto compute_type = CUSPARSE_COMPUTE_TF32;\n";
...
static int alg_id;
        ...
        lu << "\t CHECK_CUSPARSE( cusparseLtSpMMAPrune(&cusparselt_handle, &matmul, input1, input1, CUSPARSELT_PRUNE_SPMMA_TILE, stream) );\n";
        lu << "\t CHECK_CUSPARSE( cusparseLtSpMMAPruneCheck(&cusparselt_handle, &matmul, input1, d_valid, stream) );\n";
        ...
        lu << "\t CHECK_CUSPARSE( cusparseLtSpMMACompress(&cusparselt_handle, &plan, input1, d_compressed, stream) );\n";
        lu << "\t CHECK_CUSPARSE( cusparseLtMatmulSearch(&cusparselt_handle, &plan, &alpha, input0, d_compressed, &beta, output0, output0, d_workspace, streams, num_streams) );\n";
        ...
        lu << "}\n";
        lu << "CHECK_CUSPARSE( cusparseLtMatmul(&cusparselt_handle, &plan, &alpha, input0, d_compressed, &beta, output0, output0, d_workspace, streams, num_streams) );\n";
    }
