...
namespace nnfusion
{
    namespace kernels
    {
        namespace cuda
        {
            class RocmConvolutionCudnnInt8 : public CudaLibEmitter
            {
            public:
                RocmConvolutionCudnnInt8(shared_ptr<KernelContext> ctx)
                    : CudaLibEmitter(ctx)
                {
                }
    ...
    int C_pad = ((@CI@ + 3) / 4) * 4;
    CUDNN_SAFE_CALL(miopenSet4dTensorDescriptor(tensor_desc_0, miopenInt8x4, @N@, C_pad, @H@, @W@));
    CUDNN_SAFE_CALL(miopenSet4dTensorDescriptor(tensor_desc_1, miopenInt32, @N@, @CO@, @HO@, @WO@));
    CUDNN_SAFE_CALL(miopenSet4dTensorDescriptor(filter_desc, miopenInt8x4, @CO@, C_pad, @KH@, @KW@));
	...
	if (!inited) {
        ...
        assert(0 == miopenConvolutionForwardGetWorkSpaceSize(cudnn_handle, filter_desc, tensor_desc_0, conv_desc, tensor_desc_1, &workspace_size_in_bytes));
	...
	CUDNN_SAFE_CALL(miopenConvolutionForward(cudnn_handle,
		&alpha, tensor_desc_0, input0, filter_desc, input1, conv_desc, miopenConvolutionFwdAlgoGEMM,
...
REGISTER_KERNEL_EMITTER(
    "ConvolutionInt8",                                                                 // op_name
    Device(ROCM_GPU).TypeConstraint(element::f32).Tag("cudnn_kernel").Priority(5), // attrs
    cuda::RocmConvolutionCudnnInt8)                                                    // constructor
