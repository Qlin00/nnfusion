    ...
    if (m_function_unit != nullptr)
    {
        ...
        if (gnode && (*gnode)["Async_info"].is_valid())
        {
            ...
            if (stream)
            {
                ...
                if (sig_unit->get_code().find("cusparseLtHandle_t") != string::npos)
                {
                    NNFUSION_CHECK(binding_symbol.find("cusparselt_handle") != binding_symbol.end());
                    lu << binding_symbol["cusparselt_handle"] << ",";
                }
