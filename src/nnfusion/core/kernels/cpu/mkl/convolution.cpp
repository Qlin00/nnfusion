...
cpu::ConvolutionMkl::ConvolutionMkl(shared_ptr<KernelContext> ctx)
...
LanguageUnit_p cpu::ConvolutionMkl::emit_function_body()
{
    ...
    auto dnnl_engine = * global_engine_ptr;
    auto engine_stream = * global_dnnl_stream;
    ...
    const memory::dim N = @batch_count@, // batch size
        ...
        auto user_src_mem = memory({src_dims, dt::f32, tag::nchw}, dnnl_engine, (void*) input0);
        auto user_weights_mem = memory({weights_dims, dt::f32, tag::oihw}, dnnl_engine, (void*) input1);
        auto user_dst_mem = memory({dst_dims, dt::f32, tag::nchw}, dnnl_engine, (void*) output0);
        ...
        // auto user_bias_mem = memory(user_bias_md, dnnl_engine, (void*) input2);
        ...
        auto conv_pd
                = convolution_forward::primitive_desc(conv_desc, dnnl_engine);
        ...
        if (conv_pd.src_desc() != user_src_mem.get_desc()) {
            conv_src_mem = memory(conv_pd.src_desc(), dnnl_engine);
        ...
        if (conv_pd.weights_desc() != user_weights_mem.get_desc()) {
            conv_weights_mem = memory(conv_pd.weights_desc(), dnnl_engine);
        ...
        if (conv_pd.dst_desc() != user_dst_mem.get_desc()) {
            conv_dst_mem = memory(conv_pd.dst_desc(), dnnl_engine);
        }
        ...
        // conv_args.insert({DNNL_ARG_BIAS, NULL});
...
LanguageUnit_p cpu::ConvolutionMkl::emit_dependency()
{
...
REGISTER_KERNEL_EMITTER(
    ...
    cpu::ConvolutionMkl)                                                     // constructor
