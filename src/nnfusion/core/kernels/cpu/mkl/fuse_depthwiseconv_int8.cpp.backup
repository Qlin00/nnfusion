...
#include "fuse_depthwiseconv.hpp"
...
cpu::FuseDepthConvolutionMkl::FuseDepthConvolutionMkl(shared_ptr<KernelContext> ctx)
    : MklKernelEmitter(ctx)
{
    ...
    const auto& padding_above_h = int64_t(_op->localOpConfig.getRoot()["padding_after"][0]);
    const auto& padding_above_w = int64_t(_op->localOpConfig.getRoot()["padding_after"][1]);
    ...
    window_dilation_strides = nnfusion::Strides(vector<size_t>({dilation_h, dilation_w}));
    window_movement_strides = nnfusion::Strides(vector<size_t>({stride_h, stride_w}));
    data_dilation_strides = nnfusion::Strides();
    padding_below_diff = _op->localOpConfig.getRoot()["padding_before"];
    padding_above_diff = _op->localOpConfig.getRoot()["padding_after"];
    data_format = _op->localOpConfig.getRoot()["data_format"];;
...
LanguageUnit_p cpu::FuseDepthConvolutionMkl::emit_function_body()
{
    ...
    auto& lu = *_lu;
    size_t group = filter_shape[3] * filter_shape[2];
    ...
    size_t filter_count = filter_shape[2];
    size_t kernel_height = filter_shape[0];
    size_t kernel_width = filter_shape[1];
    ...
    auto conv_pd
            = convolution_forward::primitive_desc(conv_desc, conv_attr, my_engine);
...
LanguageUnit_p cpu::FuseDepthConvolutionMkl::emit_dependency()
{
...
REGISTER_KERNEL_EMITTER(
    "FuseDepthwiseConv2dNative",                                                            // op_name
    Device(GENERIC_CPU).TypeConstraint(element::f32).Tag("mkl").Priority(7), // attrs
    cpu::FuseDepthConvolutionMkl)                                                     // constructor
