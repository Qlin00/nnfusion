...
HipSparseDot::HipSparseDot(size_t reduction_axes_count,
...
HipSparseDot::HipSparseDot(shared_ptr<Dot> ori_dot,
                     int dim_m,
                     int dim_k,
                     int dim_n,
                     int sparse_nnz)
    ...
    , m_dim_n(dim_n)
    , m_sparse_nnz(sparse_nnz)
// Initialize the HipSparseDot Op according to the original Dot Op
{
...
void HipSparseDot::validate_and_infer_types(std::shared_ptr<graph::GNode> gnode)
{
