...
public:
    ...
    void load_quantize_cfg()
    {
        ...
        while (std::getline(cfg_file, line))
        {
            ...
            std::cout << "Layer:" << name << " bit:" << n_bit << " row_file:" << row_file
                      << " col_file:" << col_file << std::endl;
        ...
        for (auto node : gnodes)
        {
            ...
            if (node->get_op_type() == "Dot")
            {
                ...
                for (auto in_edge : node->get_in_edges())
                {
                    ...
                    {
                        has_bias = true;
                        std::cout << " IsLinear:" << has_bias
                                  << " Shape:" << src_node->get_output_shape(0);
                        std::cout << std::endl;
                    }
                }
            }
            ...
            if (node->get_op_type() == "Dot")
            {
                fused_op = get_dot_fuse_option(node);
                     ...
                     node->get_op_type() == "DepthwiseConv2dNative")
            {
            ...
            else if (node->get_op_type() == "Convolution")
            {
                ...
                if (quantize_bit == 8)
                {
                    ...
                    {
                        NoBlockConv1x1QuantizeOptimize8bit(
                            node, n_device_type, kernel_entry, fused_op);
                    }
                }
            }
                                     ...
                                     vector<std::shared_ptr<GNode>> fused_ops)
    {
        ...
        auto weight_values_node =
            create_constant_node(dt, ori_device_id, w_shape, block_weight_values);
                                       ...
                                       vector<std::shared_ptr<GNode>> fused_ops)
    {
        ...
        if (need_converter)
        {
            ...
            auto convert_scale_integer_node =
                std::make_shared<GNode>(convert_scale_integer, GNodeVector({}));
            convert_scale_integer->revalidate_and_infer_types(
                convert_scale_integer_node->shared_from_this());
            ...
            auto convert_scale_shift_node =
                std::make_shared<GNode>(convert_scale_shift, GNodeVector({}));
            convert_scale_shift->revalidate_and_infer_types(
                convert_scale_shift_node->shared_from_this());
    ...
    nnfusion::cache::KernelEntry_p fetch_kernel(shared_ptr<cache::KernelCacheManager> cache_manager,
                                                shared_ptr<GNode> node,
                                                NNFusion_DeviceType devtype)
    {
        std::cout << "Fetching kernel for" << node->get_unique_name() << " " << node->get_name()
                  << std::endl;
        std::shared_ptr<KernelContext> ctx(new KernelContext(node));
        std::string identifier = ctx->generate_identifier();
        std::cout << "Identifier: " << identifier << std::endl;
                                 ...
                                 vector<std::shared_ptr<GNode>> fused_ops)
    {
        ...
        for (auto in_edge : cur_node->get_in_edges())
        {
            ...
            if (src_node->is_constant())
            {
                ...
                float* block_weight_values = (float*)malloc(sizeof(float) * weight_count);
                load_from_file(
                    (char*)block_weight_rows, weight_count, csr_row[cur_node->get_name()]);
                load_from_file(
                    (char*)block_weight_cols, weight_count, csr_col[cur_node->get_name()]);
                load_from_file(
                    (char*)block_weight_values, weight_count, csr_values[cur_node->get_name()]);
                ...
                auto weight_values =
                    std::make_shared<op::Constant>(from<float>(),
                                                   nnfusion::Shape(w_shape),
                                                   static_cast<void*>(block_weight_values));
