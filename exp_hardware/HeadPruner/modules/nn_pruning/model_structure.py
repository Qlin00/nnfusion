...
class ModelStructure:
    ...
    def get_position_ffn(cls, module_name):
        for i, pattern in enumerate(cls.FFN_LAYERS):
            if cls.LAYER_PATTERNS[pattern] in module_name:
                return i
        FFN_PATTERNS_STRING = ", ".join(
            [f"{v}" for k, v in cls.LAYER_PATTERNS.items() if k in cls.FFN_LAYERS]
        )
        raise RuntimeError(
            f"Module name {module_name} does not match any of the FFN patterns : {FFN_PATTERNS_STRING}"
        )
...
class BertStructure(ModelStructure):
    ...
    LAYER_PATTERNS = dict(
        ...
        att_weight_mask="attention.self.att_weight_mask",
...
def struct_from_config(config):
    ...
    if structure is None:
        raise ModelStructureNotFound(
            f"Model config does not match any of the defined structures."
        )
    return structure
...
def struct_from_name(model_name):
    ...
    raise ModelStructureNotFound(
        f"Model name {model_name} does not match any of the defined structures."
    )
