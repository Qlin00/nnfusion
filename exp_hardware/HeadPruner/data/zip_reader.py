...
class ZipReader(object):
    ...
    def list_folder(path):
        ...
        for file_folder_name in zfile.namelist():
            ...
            if (
                file_folder_name.startswith(folder_path)
                and len(os.path.splitext(file_folder_name)[-1]) == 0
                and file_folder_name != folder_path
            ):
    ...
    def list_files(path, extension=None):
        ...
        for file_folder_name in zfile.namelist():
            ...
            if (
                file_folder_name.startswith(folder_path)
                and str.lower(os.path.splitext(file_folder_name)[-1]) in extension
            ):
                if len(folder_path) == 0:
