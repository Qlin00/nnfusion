...
def kernel_execution(kernel: str) -> Tuple[str, float, bool]:
    ...
    file_name_new = "kernel_balance_align_shared_int8.cu"
...
def run_kernel(config):
    template_name = os.path.join(current_path, "balance_align_shared_int8.cu")
...
def main():
    test_cases = [[256, 1024, 4096],[1024, 1024, 4096], [4096, 1024, 4096],\
    [256, 4096, 1024], [1024, 4096, 1024], [4096, 4096, 1024],\
    [256,5120,20480], [1024,5120,20480], [4096,5120,20480], \
    [256,20480,5120], [1024,20480,5120], [4096,20480,5120]]
